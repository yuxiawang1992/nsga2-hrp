...
计算目标函数的第二种方法，在计算经过的格网时采用了缓存的方法，看看哪个计算快一些吧~~~
@author: dell
"""
...
#目标函数
def objFunction(vardim, chrom, bound,alpha):
    ...
    objValue = alpha * (original_ave_dis/temp_average_distance) + (1-alpha)* (float(original_grid_outliers)/grid_outliers)
    
    
    print("优化的医院ID为："+str(hos_opt_id_arr[0:vardim]))
    print("平均距离为："+str(temp_average_distance))
    print('离群点数目：'+str(grid_outliers))
    print("计算出来的目标函数的值为："+str(objValue))
    
    
    return objValue,temp_average_distance,grid_outliers,hos_opt_id_arr[0:vardim],grid_density
